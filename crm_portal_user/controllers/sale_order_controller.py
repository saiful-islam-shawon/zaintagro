...
class SaleOrder(http.Controller):
    ...
    def sale_order(self, **kwargs):
        ...
        product_tax_ids_map = {
            product.id: ",".join(str(tax.id) for tax in product.taxes_id)
            for product in products
        }
        
        SaleOrderModel = request.env["sale.order"].sudo()
        payment_method_field = SaleOrderModel._fields["preferred_payment_method_line_id"]
        PaymentMethodModel = request.env[payment_method_field.comodel_name].sudo()

        payment_method_domain = []

        if "active" in PaymentMethodModel._fields:
            payment_method_domain.append(("active", "=", True))
        ...
        payment_methods = PaymentMethodModel.search(payment_method_domain, order="name")
        ...
        invoice_journals = request.env["account.journal"].sudo().search([
            ("type", "=", "sale"),
            ("company_id", "=", company_id),
        ], order="name")
        ...
        sale_tag_field = SaleOrderModel._fields.get("tag_ids")
        sale_tags = (
            request.env[sale_tag_field.comodel_name].sudo().search([], order="name")
            if sale_tag_field
        ...
        default_salesperson = request.env["res.users"].sudo().browse(
            sale_defaults.get("user_id")
        ).exists()
        
        default_sales_team = request.env["crm.team"].sudo().browse(
            sale_defaults.get("team_id")
        ).exists()
        
        default_warehouse = request.env["stock.warehouse"].sudo().browse(
            sale_defaults.get("warehouse_id")
        ).exists()
        ...
        values = {
            ...
            "salespersons": request.env["res.users"].sudo().search(
            ...
            "fiscal_positions": request.env["account.fiscal.position"].sudo().search(
                ["|", ("company_id", "=", False), ("company_id", "=", company_id)],
                order="name",
            ),
            ...
            "incoterms": request.env["account.incoterms"].sudo().search([], order="name"),

            "picking_policies": request.env["sale.order"]._fields["picking_policy"].selection,
            ...
            "utm_campaigns": request.env["utm.campaign"].sudo().search([], order="name"),
            "utm_mediums": request.env["utm.medium"].sudo().search([], order="name"),
            "utm_sources": request.env["utm.source"].sudo().search([], order="name"),                                                                                      
        }
    ...
    def submit_sale_order(self, **kwargs):
        try:
            lines_payload = json.loads(kwargs.get("order_lines_json") or "[]")
        except ValueError:
            lines_payload = []
        ...
        for line in lines_payload:
            ...
            tax_ids = [int(tax_id) for tax_id in line.get("tax_ids", [])]
            discount = float(line.get("discount") or 0)
            line_subtotal = price_unit * qty * (1 - discount / 100)
        ...
        global_discount_value = float(kwargs.get("global_discount_value") or 0)

        global_discount_amount = 0

        if discount_mode == "global" and global_discount_value > 0:
            global_discount_amount = untaxed_total * (global_discount_value / 100)

        elif discount_mode == "fixed" and global_discount_value > 0:
            global_discount_amount = min(global_discount_value, untaxed_total)
        ...
        if commitment_date:
            commitment_date = commitment_date.replace("T", " ")
            if len(commitment_date) == 16:
                commitment_date += ":00"
            commitment_date = fields.Datetime.to_datetime(commitment_date)
        ...
        order_vals = {
            ...
            "user_id": int(kwargs["user_id"]) if kwargs.get("user_id") else False,
            "team_id": int(kwargs["team_id"]) if kwargs.get("team_id") else False,
            "company_id": int(kwargs["company_id"]) if kwargs.get("company_id") else request.env.company.id,
            "require_signature": bool(kwargs.get("require_signature")),
            "require_payment": bool(kwargs.get("require_payment")),
            "client_order_ref": kwargs.get("client_order_ref") or False,
            "fiscal_position_id": int(kwargs["fiscal_position_id"]) if kwargs.get("fiscal_position_id") else False,
            "warehouse_id": int(kwargs["warehouse_id"]) if kwargs.get("warehouse_id") else False,
            "incoterm": int(kwargs["incoterm"]) if kwargs.get("incoterm") else False,
            "incoterm_location": kwargs.get("incoterm_location") or False,
            "picking_policy": kwargs.get("picking_policy") or False,
            "commitment_date": commitment_date or False,
            "origin": kwargs.get("origin") or False,
            "opportunity_id": int(kwargs["opportunity_id"]) if kwargs.get("opportunity_id") else False,
            ...
            "preferred_payment_method_line_id": int(kwargs["preferred_payment_method_line_id"]) if kwargs.get("preferred_payment_method_line_id") else False,
            "journal_id": int(kwargs["journal_id"]) if kwargs.get("journal_id") else False,
        }
        
        SaleOrderModel = request.env["sale.order"].sudo()
        if "tag_ids" in SaleOrderModel._fields:
            order_vals["tag_ids"] = [(6, 0, tag_ids)]
            

        SaleOrderModel.create(order_vals)
